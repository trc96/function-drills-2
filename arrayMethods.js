...
const taxedPrices = (prices, taxRate) => {
  const postTaxPrices = prices.map(postTax => postTax * (1 + taxRate))
  return postTaxPrices
}
// console.log(taxedPrices(prices, .07))
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(largeMonster => largeMonster.CP > 200)
...
// CODE HERE
// const totalPrice = orders.map ((price, tax) => {
//   let taxedPrice = price * (1 + tax)
//   return taxedPrice
// })
...
//   // let taxedPrice = price * (1 + tax)
//   const order = price * (1 + tax)
//   const taxedPrice = orders.map(order)
//   return taxedPrice
// }
...
// const bobSales = purchases.filter(salesOwner => salesOwner.owner == "Bob")
// console.log(bobSales)

const bobPrices = purchases.filter(purchases => purchases.owner === "Bob").reduce ((a, cv) => a + cv.price, 0)
